...
// 履歴(最近開いたファイル)を複数端末間で共有するための同期用HTTPクライアント。
// BookmarkSyncClientと同じ流儀・同じサーバー(bookmark-sync-server)の別エンドポイント(/history)を使う。
// 栞と違い1ファイルにつき常に1件のみ(開き直すたびに上書き)。push系はベストエフォートで、
// 失敗してもローカルの履歴データには影響しない。
public class HistorySyncClient {
    private static final String TAG = "HistorySyncClient";

    // サーバー側のJSONスキーマに対応するDTO(bookmarksと同じ形)
    ...
    private static HistoryDto toDto(RecordItem item, String host) {
    ...
    /**
     * 履歴の追加/更新をサーバーへ反映する(ベストエフォート、fire-and-forget)。
            ...
            public void run() {
                ...
                try {
                    ...
                    if (responseCode != HttpURLConnection.HTTP_OK) {
                        Logcat.e(logLevel, "HistorySync push failed: HTTP " + responseCode);
                    }
                }
                catch (Exception e) {
                    // ベストエフォート: ローカルの履歴は既に保存済みなので、同期失敗はログのみ
                    Logcat.e(logLevel, "HistorySync push error", e);
                }
                finally {
                    if (conn != null) {
                        conn.disconnect();
    ...
    /**
     * 履歴の削除をサーバーへ反映する(ベストエフォート、fire-and-forget)。
     */
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                try {
                    ...
                    int responseCode = conn.getResponseCode();
                    if (responseCode != HttpURLConnection.HTTP_OK) {
                        Logcat.e(logLevel, "HistorySync delete failed: HTTP " + responseCode);
                    }
                }
                catch (Exception e) {
                    Logcat.e(logLevel, "HistorySync delete error", e);
                }
                finally {
                    if (conn != null) {
                        conn.disconnect();
    ...
    /**
     * 指定ホストの履歴一覧をサーバーから取得する。結果はHandler経由で返す
     * (msg.what = DEF.HMSG_HISTORYSYNC_RESULT, msg.arg1 = server番号, msg.obj = ArrayList&lt;RecordItem&gt;)。
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                try {
                    ...
                    HistoryDto[] dtos = new Gson().fromJson(response.toString(), HistoryDto[].class);
                    ...
                    Message msg = handler.obtainMessage(DEF.HMSG_HISTORYSYNC_RESULT, server, 0, list);
                    handler.sendMessage(msg);
                }
                catch (Exception e) {
                    Logcat.e(logLevel, "HistorySync pull error", e);
                }
                finally {
                    if (conn != null) {
                        conn.disconnect();
